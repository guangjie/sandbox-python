...
def ask_questions(questions):
    ...
    answers['quantity'] = str(compute_leave_days_applied(answers['start_date'], answers['end_date']))
    return answers
...
def ask_confirm(answers, questions):
    ...
    question = [
        inquirer.Confirm(
            'confirm',
            message="Is the information good to apply?",
            default=True
        )
    ]
...
def set_worksheet(answers, data):
    wb = Workbook()
    ws = wb.active
...
def compute_leave_days_applied(str_start_date, str_end_date):
...
def main():
    ...
    set_worksheet(answers, data)
    send_email(answers, data)
